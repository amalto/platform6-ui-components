...
class SelectText extends React.Component<SelectText.Props, SelectText.State> {
  ...
  componentDidMount() {
    ...
    if (this._input) {
      this._input.value = displayLabel?.toString() ?? '';
      this.setState({ options: this.autocompleteOptions(this._input.value) });
    }
  }

  componentDidUpdate(prevProps: SelectText.Props, prevState: SelectText.State) {
    const { displayLabel, selectOpen } = this.state;

    loadTooltips(this._body);
    if (prevState.selectOpen !== selectOpen && !selectOpen && this._input) {
      this._input.value = displayLabel?.toString() ?? '';
    }
  ...
  render() {
    ...
    const autocompleteInput = (value: string) =>
      this.setState({ options: this.autocompleteOptions(value) });
    const inputDisabled: boolean = disabled || options?.length === 0;
    ...
    return (
      ...
      >
        ...
        />
        <Radium.Style
          scopeSelector=".option-item-disabled"
          rules={Styles['option-item-disabled']}
        />
        ...
        <div className="select-text-input">
          <input
            ...
            autoCapitalize="off"
            autoFocus={autofocus}
            className={classNames('form-control padr-20', inputClass, {
          ...
          />
          {selectOpen ? this.renderOptions() : null}
        </div>
      </div>
    );
  }
    ...
    this.setState(
      {
        ...
        options: this.state.lockFocus
          ? this.state.options
          : this.autocompleteOptions(this.state.displayLabel.toString()),
  ...
  private handleKeyboardShortcut = (event) => {
    switch (event.keyCode) {
      ...
      case 38: {
        ...
        const selectedOption: number = options.findIndex(
          (opt) => opt.value.toString() === displayValue.toString(),
        );
      ...
      case 40: {
        ...
        this.setState(
          {
            displayValue: options[nextOption]?.value.toString() ?? displayValue,
            displayLabel: options[nextOption]?.label.toString() ?? displayLabel,
          },
          () => this.scrollToOption(this.state.displayValue.toString()),
        );
        break;
