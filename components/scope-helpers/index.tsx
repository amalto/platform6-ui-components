...
export function hasAnyPermission(
  ...
  for (let i = 0; i < permissions.length; i++) {
    if (typeof permissions[i] === 'object') {
      ...
      const appInstanceValues = getScopeValues(
        webStorage,
        global ? ASTERISK : appInstance,
        (permissions[i] as PermissionDef).feature,
        (permissions[i] as PermissionDef).action,
      );
      ...
      if (appInstanceValues) {
        ...
        if ((appInstanceValues as string[]).length) {
          if (
            (appInstanceValues as string[]).indexOf(
              (permissions[i] as PermissionDef).requiredValue,
            ) !== -1
          ) {
            return true;
          }
      ...
      if (global) {
        ...
        if (asteriskValues) {
          ...
          if ((asteriskValues as string[]).length) {
            if (
              (asteriskValues as string[]).indexOf(
                (permissions[i] as PermissionDef).requiredValue,
              ) !== -1
            ) {
              return true;
...
export function getScopeValues(
  ...
  return (
    scopesTree &&
    scopesTree[appInstance] &&
    scopesTree[appInstance][feature] &&
    scopesTree[appInstance][feature][action]
  );
}
