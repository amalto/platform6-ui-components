...
class MultiSelect extends React.Component<MultiSelect.Props, MultiSelect.State> {
    ...
    private handleSelectChange = ( event: any ) => {
        if ( this.props.multiple ) { $( event.currentTarget ).scrollTop( 0 ) }

        this.props.handleChange( event )
    }

    private renderDropdownInput = ( filterDisplayName: string | JSX.Element, value?: string | string[], optionsLabel?: string[] ) => {
        const { multiple, fieldLineHeight, options, label, help, inputClass, containerClass } = this.props
            ...
            let choices = ( optionsLabel as string[] ).map( ( choice, idx ) => {
                ...
                return (
                    <li key={idx} className={classNames( { 'active': value.indexOf( option ) !== -1 } )}>
                        <a href="#" onClick={( e ) => { e.preventDefault(); this.handleChoiceSelect( selectValue, option, multiple, e.ctrlKey || e.metaKey ) }}>{option}</a>
                    </li>
                )
            } )
                                ...
                                <div className="dropdown-active-tags">
                                    {
                                        selectValue.map( ( value, idx ) => {
                                            ...
                                            return (
                                                <span key={value + idx} className="tag" onClick={() => this.handleChoiceSelect( selectValue, value, multiple )}>
                                                    <span>{value}</span>
