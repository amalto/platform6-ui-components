...
class FileInput extends React.Component<FileInput.Props, FileInput.State> {
  ...
  render() {
    ...
    const filesDisplay = files
      .filter((file) => file.sourceFile !== undefined)
      .map((file, idx) => (
        <FileWrapperDisplay
          key={idx}
    ...
    let dropzone = (
      <Dropzone
        className="col-xs-12 file-drop-zone"
        onDrop={this.onDrop}
        disableClick={this.state.submitDisabled}
        accept={this.props.mimeTypeAccepted}
        maxSize={this.props.maxBytesSize}
      >
        <div className="drop-zone-title">
          <span>{wordings.dropZoneTitle}</span>
          <br />
    ...
    let cancelBtn = (
      <div className="top-margin">
        <button
          type="button"
          className="btn btn-sm btn-danger btn-trans"
    ...
    let errors = this.state.invalidFiles.map((file, idx) => {
      return (
        <div key={idx}>
          ...
          <em>
            {formatFileSize(file.size)}, {file.type || wordings.unknownFormat}
          </em>
        </div>
      );
    });
    ...
    return (
      <div className="file-input">
        <div className="row">
          <div className="col-xs-12 col-sm-5">{dropzone}</div>

          <div className="col-xs-12 col-sm-7">
            <div className="file-list">
              <ul style={{ margin: 0 }}>{filesDisplay}</ul>

              {this.state.invalidFiles.length ? (
                <div className="alert alert-danger alert-dismissable top-margin text-medium">
                  <button type="button" className="close" onClick={this.closeInvalidFilesAlert}>
                    &times;
                  </button>
                  <div className="bottom-spaced">
                    <strong className="right-spaced">{wordings.invalidFile}</strong>
                    <span
                      className="fas fa-info-circle click-pointer"
                      data-toggle="collapse"
                      data-target="#inputFormatInfo"
                    />
                    <div
                      id="inputFormatInfo"
                      className="collapse top-spaced font-color"
                      style={{ paddingLeft: 20 }}
                    >
                      {this.props.mimeTypeAccepted ? (
                        <div>
                          <strong className="right-spaced">{wordings.contentType}</strong>
                          <span>{this.props.mimeTypeAccepted}</span>
                        </div>
                      ) : null}
                      {this.props.maxBytesSize ? (
                        <div>
                          <strong className="right-spaced">{wordings.maxSize}</strong>
                          <span>{formatFileSize(this.props.maxBytesSize)}</span>
                        </div>
                      ) : null}
                    </div>
                  </div>
