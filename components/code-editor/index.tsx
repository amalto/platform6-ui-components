...
class CodeEditor extends React.Component<CodeEditor.Props, any> {
  ...
  private _cursorLastPosition: { row: number; column: number } = {
    row: 0,
    column: 0,
  };
  ...
  componentWillUnmount() {
    ...
    this.props.saveSession &&
      this.props.saveSession(
        $.extend({}, this.getAceSession(this._editor), {
          cursorPosition: this._cursorLastPosition,
        }),
      );
  ...
  private setEditorOptions = (editor: AceEditor, props: CodeEditor.Props) => {
    ...
    editor.setTheme(
      (displaySettings.theme && `ace/theme/${displaySettings.theme}`) ||
        userSettings.theme ||
  ...
  private setKeyboardShortcuts = (
    ...
    editor.commands.addCommand({
      ...
      exec: (currentEditor) => {
        props.saveContent &&
          props.saveContent(
            $.extend({}, this.extractAceSession(currentEditor), {
