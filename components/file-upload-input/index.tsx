...
import { formatFileSize, compileWordings } from '@amalto/helpers';
...
class FileInput extends React.Component<FileInput.Props, FileInput.State> {
  ...
  componentDidUpdate(prevProps: FileInput.Props) {
    if (prevProps.filename !== this.props.filename)
      this.setState({ filename: this.props.filename } as FileInput.State);
    if (prevProps.filesize !== this.props.filesize)
      this.setState({ filesize: this.props.filesize } as FileInput.State);
  }
      ...
      >
        ...
        <div className="form-control pos-relative">
          ...
          {!!filename && !!filesize ? (
            <span className="italic">
              ...
              ,{' '}
              <span>
                <span className="black-color bold">{wordings.size}:</span>{' '}
                {formatFileSize(filesize)}
              </span>
            </span>
          ) : !fileUploaded ? (
            <span className="italic">{wordings.noFileChosen}</span>
          ) : (
            <span className="italic">{wordings.fileUploaded}</span>
          )}
        </div>

        {meta.touched && !!meta.error ? (
          <p className="validation-error-message">{meta.error}</p>
        ) : collapseErrorSpace || this.state.loadingError ? null : (
        ...
        {input.value && displayPreview && fileContent ? (
          <div className="hidden-xs top-spaced">
            <em>
              <small>
                {label && <span className="right-spaced">{label}</span>}
                {wordings.previewLowerCase}
              </small>
            </em>
            {getAsDataUrl ? (
              <div>
                <img style={{ maxHeight: 150 }} src={fileContent} />
              </div>
            ) : (
