...
function defaultGetValue<T extends DataItem>(
  data: T,
...
function defaultSetValue<T extends DataItem>(
  value: string,
  data: T,
  context: ColumnDefinition<T>
): T {
...
let columnIds = 0;
export function fromDefinition<T extends DataItem>(
...
export function replaceColumn<T extends DataItem>(
  updatedColumn: Column<T>,
  allColumns: Column<T>[]
): Column<T>[] {
...
export function move<T extends DataItem>(
  id: ColumnId,
  direction: Direction,
  columns: Column<T>[]
): Column<T>[] {
  ...
  return result
    .slice(0, to)
    .concat(item)
    .concat(result.slice(to, columns.length));
}
