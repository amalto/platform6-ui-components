...
import { P6Radio } from "~components/atoms/p6-radio/p6-radio";
...
function isHTMLP6RadioElement(
  value: HTMLElement | EventTarget
): value is HTMLP6RadioElement {
...
export class P6RadioGroup implements ComponentInterface {
  ...
  render(): void {
    return (
      <Host
        onP6FormRegister={this.addRadioControl}
        onP6FormUnregister={this.removeRadioControl}
        onClick={this.updateRadioControls}
      >
        <slot />
      </Host>
    );
  ...
  private updateRadioControls = (event: Event): void => {
    if (event.target !== null && isHTMLP6RadioElement(event.target)) {
      ...
      this.radioControls.forEach((radio) => {
        if (
          radio.name === radioClicked.name &&
          radio.value !== radioClicked.value
        ) {
  ...
  private removeRadioControl = (
    event: CustomEvent<P6Control<unknown>>
  ): void => {
    this.radioControls = this.radioControls.filter(
      (radio) => radio === event.detail
    );
  };
}
